    ...
    const fetchData = async () => {
      try {
        ...
        if (res.ok) {
          ...
          toast.error('السلايد غير موجود');
          router.push('/admin/slides');
        }
      } catch (error) {
        toast.error('فشل تحميل بيانات السلايد');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const onSubmit = async (data: SlideFormData) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error(errorData.error || 'فشل تحديث السلايد');
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">تعديل السلايد</h1>
            <p className="text-gray-600 mt-1">قم بتعديل بيانات شريحة العرض</p>
          </div>
        </div>
      </div>
