...
export default function EditTVInterviewPage() {
  ...
  const { register, handleSubmit, setValue, reset, formState: { errors } } = useForm<InterviewFormData>({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`/api/admin/tv-interviews/${interviewId}`);
        if (res.ok) {
          ...
          toast.error('المقابلة غير موجودة');
  ...
  const onSubmit = async (data: InterviewFormData) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error('فشل تحديث المقابلة');
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link href="/admin/tv-interviews"><Button variant="ghost" size="icon"><ArrowRight size={20} /></Button></Link>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">تعديل المقابلة</h1>
          <p className="text-gray-600 mt-1">قم بتعديل بيانات المقابلة</p>
        </div>
      </div>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div>
                      ...
                      <Input id="channelAr" {...register('channelAr')} className="mt-1.5" />
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <Input id="videoUrl" {...register('videoUrl')} className="mt-1.5" dir="ltr" />
