  ...
  const onSubmit = async (data: NewsFormData) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error(errorData.error || 'فشل إنشاء الخبر');
      }
    } catch (error) {
      toast.error('حدث خطأ أثناء إنشاء الخبر');
    } finally {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">إضافة خبر جديد</h1>
            <p className="text-gray-600 mt-1">أضف خبر جديد للشركة</p>
          </div>
        </div>
      </div>
